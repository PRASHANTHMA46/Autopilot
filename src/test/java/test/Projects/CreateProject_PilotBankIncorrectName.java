...
public class CreateProject_PilotBankIncorrectName extends BaseTest {
	...
	@Test(priority = 16)
	public void verify_SaveEditLanuch_IncorrectName() throws EncryptedDocumentException, IOException, InterruptedException {
		// basic details enter to continue
		CreateProject cp = new CreateProject(driver);
		String Enteronlyalphabets = ExcelData.getData("projects", 42, 2);
		cp.Enter_ProjectTitle(Enteronlyalphabets);
		String Enterdescription = ExcelData.getData("projects", 43, 2);
		...
		Thread.sleep(1000);
		//Edit the title
		String Enterdescription2 = ExcelData.getData("projects", 43, 2);
		cp.Clear_ProjectDescription(Enterdescription2);
	...
	@Test(priority = 17)
	public void verify_PilotBankIncorrectName()
			throws EncryptedDocumentException, IOException, InterruptedException {
		// basic details enter to continue
		CreateProject cp = new CreateProject(driver);
		String EnterincorrectName = ExcelData.getData("projects", 42, 2);
		cp.Enter_ProjectTitle(EnterincorrectName);
		String Enterdescription = ExcelData.getData("projects", 43, 2);
		cp.Enter_ProjectDescription(Enterdescription);
		cp.ClickOnCreateNewTest();
		//verify Bank instrument
		...
		Thread.sleep(2000);
		String expected_projectname = EnterincorrectName;
		...
		String expected_Transactiontype = ExcelData.getData("projects", 45, 2);
        ...
        if (status.equals("Ongoing")) {
            ...
            //Thread.sleep(2000);
            //cp.clickon_kebabiconincorrectOption();
            //cp.clickonbatchViewDetails();
        	actions.scrollByAmount(0, 700).perform();
        	Thread.sleep(3000);
        	String TestSceanrio = driver.findElement(By.xpath("//td[normalize-space()='Incorrect name']")).getText();
        	System.out.println("TestSceanrio: " + TestSceanrio );
        	String expectedscenario = ExcelData.getData("projects", 44, 2);
        	System.out.println("expectedscenario: " + expectedscenario);
        	sa.assertEquals(TestSceanrio, expectedscenario);
            cp.clickon_kebabiconincorrectOption();
            cp.clickonbatchViewDetails();
