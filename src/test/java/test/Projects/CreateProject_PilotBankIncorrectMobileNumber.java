...
public class CreateProject_PilotBankIncorrectMobileNumber extends BaseTest {
	@BeforeMethod
	...
	@Test(priority = 16)
	public void verify_SaveEditLanuch_IncorrectMobileNumber() throws EncryptedDocumentException, IOException, InterruptedException {
		// basic details enter to continue
		CreateProject cp = new CreateProject(driver);
		String Enteronlyalphabets = ExcelData.getData("projects", 38, 2);
		cp.Enter_ProjectTitle(Enteronlyalphabets);
		String Enterdescription = ExcelData.getData("projects", 39, 2);
		...
		Thread.sleep(1000);
		//Edit the title
		String Enterdescription2 = ExcelData.getData("projects", 37, 2);
	...
	@Test(priority = 17)
	public void verify_PilotBankIncorrectMobileNumber()
			throws EncryptedDocumentException, IOException, InterruptedException {
		// basic details enter to continue
		CreateProject cp = new CreateProject(driver);
		String Enteronlyalphabets = ExcelData.getData("projects", 38, 2);
		...
		String expected_Transactiontype = ExcelData.getData("projects", 41, 2);
        ...
        if (status.equals("Ongoing")) {
        	...
        	actions.scrollByAmount(0, 700).perform();
        	Thread.sleep(3000);
        	String TestSceanrio = driver.findElement(By.xpath("//td[normalize-space()='Incorrect mobile number']")).getText();
        	System.out.println("TestSceanrio: " + TestSceanrio );
        	String expectedscenario = ExcelData.getData("projects", 40, 2);
